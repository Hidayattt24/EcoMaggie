-- ===========================================
-- ADD DELIVERY LOCATION FIELDS TO TRANSACTIONS TABLE
-- ===========================================
...
-- ===========================================
-- STEP 1: ADD LOCATION COLUMNS TO TRANSACTIONS
-- ===========================================
...
COMMENT ON COLUMN public.transactions.delivery_latitude IS 'Latitude koordinat titik antar untuk Eco-Maggie delivery (-90 to 90)';
COMMENT ON COLUMN public.transactions.delivery_longitude IS 'Longitude koordinat titik antar untuk Eco-Maggie delivery (-180 to 180)';

-- ===========================================
-- STEP 3: CREATE INDEX FOR LOCATION QUERIES
-- ===========================================

CREATE INDEX IF NOT EXISTS idx_transactions_delivery_location
ON public.transactions(delivery_latitude, delivery_longitude)
...
DO $$ BEGIN
    ALTER TABLE public.transactions DROP CONSTRAINT IF EXISTS check_delivery_latitude_range;
EXCEPTION
    WHEN undefined_object THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE public.transactions DROP CONSTRAINT IF EXISTS check_delivery_longitude_range;
EXCEPTION
    WHEN undefined_object THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE public.transactions DROP CONSTRAINT IF EXISTS check_delivery_location_pair;
EXCEPTION
    WHEN undefined_object THEN NULL;
END $$;
...
/*
SUMMARY:
✅ Added delivery_latitude column (DECIMAL(10, 8)) to transactions table
✅ Added delivery_longitude column (DECIMAL(11, 8)) to transactions table
...
2. Update orders.actions.ts untuk fetch delivery_latitude dan delivery_longitude
3. Update checkout page untuk save koordinat ke transactions table
4. Update farmer orders detail page untuk tampilkan maps dengan koordinat
*/
